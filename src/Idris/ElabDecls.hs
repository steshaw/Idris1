...
import Idris.AbsSyntax
  ( getIState, putIState, getContext, setContext, updateContext
  , getErasureInfo
  , addOpenImpl, setOpenImpl , setTotality, clear_totcheck
  , addIBC, addNameHint
  , expandParamsD
  , logElab
  )
import Idris.AbsSyntaxTree
  ( PTerm(..), PDecl, PDecl'(..), PData'(..), SyntaxInfo(..)
  , pexp
  , defaultSyntax
  , declared, tldeclared
  , inferOpts, inferDecl
  , eqOpts, eqDecl, eqDoc, eqParamDoc, eqCon, eqTy
  , ElabInfo(..)
  , ElabWhat(..)
  , Idris, IState(..)
  , IBCWrite(IBCDSL)
...
elabDecl' what info (PClauses f o n ps)
  | what /= ETypes
...
elabDecl' what info (PMutual _ ps)
    = do case ps of
...
elabDecl' what info (PParams _ ns ps)
    = do i <- getIState
...
elabDecl' what info (POpenInterfaces _ ns ds)
...
-}
elabDecl' _ _    (PDSL n dsl)
    = do i <- getIState
         putIState (i { idris_dsls = addDef n dsl (idris_dsls i) })
         addIBC (IBCDSL n)
elabDecl' what _    (PDirective i)
...
elabDecl' _    info (PTransform fc safety old new)
    = do _ <- elabTransform info fc safety old new
         return ()
elabDecl' _    info (PRunElabDecl fc script ns)
